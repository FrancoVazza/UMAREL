...
#          io=findall(abs.(path[i,5,:]). > 0) 
 ...
 #  
    ...
    function plot_spectrum_suppression(path,np,root_out,E_initial,Z,cosmo,tag,root_halos)
      ...
      #...this function compares the spectrum computed for the entire volume with the spectrum measured by a limited set of observer with a finite observing sphere

      #....plotting the final spectrum 
      nn=size(path)
      npath=nn[3]
      println("size of path", npath)
        nspec=10
        mie=16.0   #...logE
        mae=21.0
        bie=(mae-mie)/nspec
         ...
         #....select observers at z=0
           ...
           @inbounds for ii in eachindex(im)#...loop over observers 
             ...
             @inbounds for j in 1:np    #...loop over particles s
              dist=sqrt( (path[j,5,iobs]-ox1)^2+(path[j,6,iobs]-oy1)^2+(path[j,7,iobs]-oz1)^2)
               if dist < radius 
                 en=path[j,4,iobs]
                 if en>10.0^mie && en<10.0^mae  
                ie=convert(Int64,trunc((log10(en)-mie)/bie))+1
